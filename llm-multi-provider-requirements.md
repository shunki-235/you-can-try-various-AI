...
- 実装フェーズの最初のステップとして、チャット画面・設定画面の **モック UI** を作成する。
- この段階では実際の LLM 呼び出しはダミーデータでよく、レイアウト・コンポーネント構造・画面遷移・操作感を優先して設計する。
- モック UI が固まり次第、本節のタスクリスト（特に 2〜7）に沿って API レイヤや認証・ストリーミング対応の実装に進む。
